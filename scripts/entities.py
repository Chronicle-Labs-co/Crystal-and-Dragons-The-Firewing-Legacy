    ...
    def update(self, tilemap, movement=(0,0)):
        ...
        for rect in tilemap.physics_rects_around(self.pos):
            if entity_rect.colliderect(rect):
                if frame_movement[0] > 0: # move to right
                    entity_rect.right = rect.left
                    self.collisions['right'] = True
                if frame_movement[0] < 0: # move to left
        ...
        for rect in tilemap.physics_rects_around(self.pos):
            if entity_rect.colliderect(rect):
                    if frame_movement[1] > 0: # move to down
                        entity_rect.bottom = rect.top
                        self.collisions['down'] = True
                    if frame_movement[1] < 0: # move to up
        ...
        self.velocity[1] = min(5, self.velocity[1] + 0.1) # if max is 5, otherwise use (self.velocity[1] + 0.1)
